...
function HiddenDungeonModal({ theme, beep, fontScale, pixelFont, setHiddenRoomOpen }) {
  ...
  return <ModalShell ariaLabel="Dungeon Journal blog" closeLabel="Close Dungeon Journal" onClose={closeJournal} panelStyle={{ maxWidth: 920, minHeight: "calc(100vh - 32px)", padding: "clamp(18px, 3vw, 24px)" }} theme={theme}>
    ...
    {selectedPost ? <article style={{ maxWidth: 700, margin: "24px auto 0" }}>
    ...
    </article> : BLOG_POSTS.length > 0 ? <section aria-label="Blog posts" className="modal-card-grid" style={{ marginTop: 18, marginBottom: 0 }}>
      {BLOG_POSTS.map(post => <PixelFrame key={post.id} as={post.href ? "a" : undefined} href={post.href} target={post.href ? "_blank" : undefined} rel={post.href ? "noopener noreferrer" : undefined} theme={theme} onClick={post.href ? () => beep(340) : () => { beep(340); setSelectedPostId(post.id); }} title={post.href || `Read ${post.title}`} style={{ padding: 16, display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 12, minHeight: 0 }}>
        ...
        <div style={{ display: "grid", gap: 3 }}>
          <h2 style={{ fontFamily: pixelFont, fontSize: `${11 * fontScale}px`, color: theme.accent, lineHeight: 1.35, margin: 0 }}>{post.title}</h2>
          <p style={{ fontFamily: "var(--copy-font)", fontSize: `${12 * fontScale}px`, color: theme.textDim, lineHeight: 1.5, margin: 0 }}>{post.excerpt}</p>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 2 }}>{post.tags.map(tag => <span key={tag} style={{ border: `1px solid ${theme.border}`, color: theme.textDim, padding: "3px 6px", fontSize: `${9 * fontScale}px` }}>{tag}</span>)}</div>
      </PixelFrame>)}
