...
import PortfolioScene from "@/components/layout/PortfolioScene";
...
export default function PortfolioHome() {
  ...
  const [showClickEgg] = useState(false);
  ...
  useEffect(() => {
    console.log("%cInspecting the code I see - nice to meet you", "font-weight: bold;");
  }, []);
  function localTime() {
      ...
      {achievementToast && <ToastNotice bottom={24 + [showClickEgg, showLevelUp, encounterMsg, nameEggShown].filter(Boolean).length * 72} fontScale={fontScale} theme={theme}>
        <div style={{ minWidth: 0, textAlign: "left", display: "flex", flexDirection: "column", justifyContent: "center", alignSelf: "center" }}>
          ...
          <div style={{ fontFamily: "var(--copy-font)", fontSize: `${11 * fontScale}px`, color: theme.text, fontWeight: 400, lineHeight: 1.25, marginTop: 2 }}>
            <span style={{ whiteSpace: "nowrap" }}>{achievementToast.label}</span> &middot; <span style={{ fontFamily: "'Pixelify Sans', 'Courier New', sans-serif", whiteSpace: "nowrap" }}>+{achievementToast.xp} XP</span>
          </div>
        </div>
      </ToastNotice>}
