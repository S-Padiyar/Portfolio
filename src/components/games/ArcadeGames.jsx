...
function ResultOverlay({ theme, children, fontScale, onRestart, title }) {
  return <div style={{ position: "absolute", inset: 0, zIndex: 5, display: "grid", placeItems: "center", background: `${theme.bg}e8`, padding: 16 }}>
    <PixelFrame theme={theme} style={{ padding: 18, textAlign: "center", maxWidth: 360 }}>
      ...
      <div style={{ color: theme.textDim, fontFamily: "var(--copy-font)", fontSize: `${12 * fontScale}px`, lineHeight: 1.55, marginBottom: 12 }}>{children}</div>
...
export function SkyboundGame({ theme, beep, fontScale }) {
  ...
  return <div>
    ...
    <div ref={arenaRef} onPointerDown={flap} style={{ position: "relative", height: GAME_HEIGHT, overflow: "hidden", touchAction: "none", cursor: "pointer", background: `linear-gradient(${theme.accentDark}44, ${theme.panelAlt} 58%, ${theme.bg})`, border: `2px solid ${theme.border}` }}>
      ...
      {status === "ready" && <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", background: `${theme.bg}88`, textAlign: "center", color: theme.text }}><div><div style={{ color: theme.accent, marginBottom: 8 }}>Skybound</div><div style={{ fontFamily: "var(--copy-font)", fontSize: `${12 * fontScale}px` }}>Tap, Space, or ↑ to fly</div></div></div>}
      {status === "over" && <ResultOverlay theme={theme} fontScale={fontScale} title="Run over" onRestart={restart}>You cleared {view.score} gates. Best: {Math.max(best, view.score)}.</ResultOverlay>}
    </div>
...
export function CoreCollectorGame({ theme, beep, fontScale }) {
  ...
  return <div>
    ...
    <div ref={arenaRef} style={{ position: "relative", height: GAME_HEIGHT, overflow: "hidden", touchAction: "none", background: `radial-gradient(circle at center, ${theme.panelAlt}, ${theme.bg})`, border: `2px solid ${theme.border}` }}>
      ...
      {over && <ResultOverlay theme={theme} fontScale={fontScale} title="System overloaded" onRestart={restart}>You recovered {view.score} cores. Best: {Math.max(best, view.score)}.</ResultOverlay>}
    </div>
