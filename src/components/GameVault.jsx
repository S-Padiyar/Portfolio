...
function VaultDoor({ theme, beep, companion, fontScale, isMobile, pixelFont, setCompanion }) {
  return <div style={{ display: "grid", justifyItems: "center", gap: 11, padding: "20px 12px" }}>
...
function DoorTransition({ theme, companion, pixelFont }) {
  ...
  return <div aria-label="Entering Game Vault" style={{ position: "fixed", inset: 0, zIndex: 10040, pointerEvents: "none" }}>
    ...
    <div style={{ position: "fixed", left: 0, right: 0, bottom: 25, textAlign: "center", color: theme.accent, fontFamily: pixelFont, fontSize: 11 }}>Entering Game Vault...</div>
  </div>;
}
...
export default function GameVault({ theme, beep, companion, enteredGameDoor, fontScale, isMobile, pixelFont, setCompanion }) {
  ...
  return <div>
    ...
    {entryActive && readyEntryId !== enteredGameDoor.entryId && <DoorTransition theme={theme} companion={companion} pixelFont={pixelFont} />}
