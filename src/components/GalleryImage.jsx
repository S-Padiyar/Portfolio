...
export default function GalleryImage({ image, onInfo, theme }) {
  const contained = image.objectFit === "contain";
  const frame = <div style={{ position: "relative", width: "100%", aspectRatio: "16 / 9", overflow: "hidden", background: theme.panelAlt, border: `1px solid ${theme.border}` }}>
    ...
    <img src={image.src} alt={image.alt} style={{ position: "relative", zIndex: 1, display: "block", width: "100%", height: "100%", objectFit: contained ? "contain" : image.objectFit || "cover", boxSizing: "border-box", transform: image.offsetX ? `translateX(${image.offsetX}px) scale(1.01)` : undefined, objectPosition: image.objectPosition || (contained ? "center" : "top") }} />
  </div>;
